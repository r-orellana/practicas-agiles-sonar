...
export class CancionService {
  ...
  comentarCancion(cancionId: number, comentario: string, token: string, parent?: number): Observable<Cancion>{
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`
    })
    return this.http.post<Cancion>(`${this.backUrl}/cancion/${cancionId}/comentario`,{contenido: comentario, parent},{headers: headers})
  }
